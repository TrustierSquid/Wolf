  ...
  for (const community of communitiesInformationCollections) {
    ...
    if (allCommunitiesSearch) {
      communityArr.push({
        ...
        image: allCommunitiesSearch.image
          ? `data:${
              allCommunitiesSearch.image.contentType
            };base64,${allCommunitiesSearch.image.data.toString("base64")}`
          : null,
      });
    }
  }
...
router.put(
  "/updateCommunityInformation/:owner/:oldName",
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (communityName) {
        ...
        await database
          .collection(`${oldName}Info`)
          .aggregate([{ $match: {} }, { $out: `${communityName}Info` }])
          .toArray();

        await database
          .collection(`${oldName}Feed`)
          .aggregate([{ $match: {} }, { $out: `${communityName}Feed` }])
          .toArray();
        ...
        console.log(
          `${owner} has changed community name! (${oldName} to ${communityName})`
        );
      }
      ...
      if (communityDescription) {
        ...
        await database
          .collection(`${communityName ? communityName : oldName}Info`)
          .updateOne(
...
router.post(
  ...
  async (req, res) => {
    ...
    console.log(
      `${communityName} was created on ${creationMonth}-${creationDay}-${creationYear}`
    );
    ...
    try {
      ...
      const foundCommunity = findCollection.some(
        (community) =>
          community.name === communityName + "Info" ||
          community.name === communityName + "Feed"
      );
      ...
      if (foundCommunity) {
        ...
        let newCommunity = await database
          .collection(communityName + "Feed")
          .insertOne(firstMessage);
        // Uploading the community info to the DB
        let newCommunityInfo = await database
          .collection(communityName + "Info")
          .insertOne(communityProps);
      ...
      console.log(
        error + " Unable to create community or community already exists"
      );
    }
  }
);
