...
export default function CommunitySelection(props) {
  ...
  async function leaveCommunity(communityToLeave, key) {
    try {
      let response = await fetch(
        `/removeCommunity/${communityToLeave}/${userLoggedInUID}/${props.username}`,
        {
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const [modifyCommunityDescription, setModifyCommunityDescription] =
    useState(null);
  ...
  async function modifyCreatedCommunity(
    communityName,
    members,
    description,
    communityImage,
  ...
  async function savingNewCommunityInformation() {
    ...
    if (
      !newCommunityName.current.value &&
      !newCommunityDescription.current.value &&
      !changePictureRef.current.files[0]
    ) {
      setErrorMessage(
        `You must modify ${modifyCommunityName} in order to save.`
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `/community/updateCommunityInformation/${communityOwner}/${modifyCommunityName}`,
        {
          method: "PUT",
          body: formData,
        }
  ...
  const renderCommunities = () => {
    if (showCreatedCommunities) {
      const findOwner = totalMembers?.filter(
        (community) => community.owner === props.username
      );
      return findOwner.length > 0 ? (
          ...
          <li key={key}>
            <section className="topHalf">
              ...
              <section className="interactionBtns">
                ...
                <button
                  className="viewCommunityPostsBtn"
                  onClick={() =>
                ...
                <button
                  className="communitySettingsBtn"
                  ref={(el) => (leaveCommunityBtn.current[key] = el)}
                  onClick={() => {
                    modifyCreatedCommunity(
                      community.name,
                      community.members,
            ...
            <section className="botHalf">
              <h4 style={{ color: "grey" }}>
                Community Created:{" "}
                {community.creationDate ? community.creationDate : "8/20/24"}
              </h4>
        ...
        <div className="noPostsMessage">
          <h3>
            You do not own any dens.{" "}
            <a href="/topics" style={{ color: "crimson" }}>
              Create one!
            </a>{" "}
          </h3>
        </div>
              ...
              <li key={key}>
                <section className="topHalf">
                  ...
                  <section className="interactionBtns">
                    ...
                    <button
                      className="leaveCommunityBtn"
                      ref={(el) => (leaveCommunityBtn.current[key] = el)}
                      onClick={() => leaveCommunity(community.name, key)}
                    >
                      Leave Community
                    </button>
                  </section>
                ...
                <section className="botHalf">
                  ...
                  <h4 style={{ color: "lightgrey" }}>
                    Owner: {community.owner ? community.owner : "WOLF Team"}
                  </h4>
                  <h4 style={{ color: "magenta" }}>
                    Members: {community.members.length}
                  </h4>
    ...
    <>
      ...
      <div id="contentContainer">
        <span
          ref={darkBG}
          id="darkBG"
          onClick={() => modifyMoodleHelper("closeMoodle")}
        ></span>
          ...
          <section id="modifyFieldCardContainer">
            <form action="" id="modifyInformation">
              ...
              <div className="modifyCard">
                ...
                <input
                  type="text"
                  ref={newCommunityName}
                  placeholder={modifyCommunityName}
                />
              </div>
              ...
              <div className="modifyCard">
                ...
                <input
                  maxLength={100}
                  type="text"
                  ref={newCommunityDescription}
                  placeholder={modifyCommunityDescription}
                />
              </div>
              ...
              <button
                ...
                ref={saveChangesBtn}
                onClick={(e) => {
                  savingNewCommunityInformation(), e.preventDefault();
                }}
              >
                Save Changes
              </button>
              <button
                ...
                onClick={(e) => {
                  modifyMoodleHelper("secondMoodle"), e.preventDefault();
                }}
              >
                Delete Community
              </button>
        ...
        <div ref={confirmMoodleRef} id="confirmMoodle">
          <h3 className="confirmMoodleTitle">
            Are sure you want to delete {modifyCommunityName}? Enter the
            community name{" "}
            <span style={{ color: "red" }}>'{modifyCommunityName}'</span> to
            confirm.
          </h3>
          <input
            type="text"
          ...
          <button
            id="deleteButton"
            onClick={() => deleteCommunity(modifyCommunityName)}
          >
            Delete
          </button>
          <p className="errorMessage" ref={confirmErrorMessage}>
            {errorMessage}
          </p>
        </div>
        ...
        <main id="communityContainer">
          <div id="titleDiv">
            ...
            <a
              onClick={() =>
                setShowCreatedCommunities((prevState) => !prevState)
              }
            >
              {showCreatedCommunities
                ? "Show all dens "
                : "Show my created dens "}
          ...
          <ul>
            {
              ...
              totalMembers ? (
                ...
                <div className="noPostsMessage">
                  <div className=" loader ">{/* Loading icon */}</div>
                </div>
              )
            }
          </ul>
