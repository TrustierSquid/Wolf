      ...
      <>
         <nav id="suggestedNav" ref={suggestedNavRef}>
            ...
            <ul id="suggestedNavMembers">
               {
                  allCommunities ? (
                     allCommunities?.filter((community) => community?.name === determinedCommunity?.split('Feed')[0]).map((community) => {
                        return (
                              ...
                              <ul key={community.name}>
                                 {
                                    Object.entries(community.members).map(([key, value]) => (
                                       <>
                                          <div key={key}>
                                             <section>
                                                <img className='' src={value.memberProfilePic ? value.memberProfilePic : 'src/assets/defaultUser.jpg'} alt="" />
                                                <h4 className="memberNames">{value.member}</h4>
                                                <button onClick={()=> navigateToProfile(value.member)}>View Profile</button>
                                             </section>
                                          </div>
                                       </>
                                    ))
            ...
            {
               allCommunities?.sort(()=> 0.5 - Math.random()).slice(0, 5).map((community)=> {
                  return (
                     <>
                        <div>
                           <section>
                              ...
                              <button onClick={()=> window.location.href = `/home?topicFeed=${community.name + 'Feed'}`} style={{color: 'turquoise', cursor: 'pointer'}}>Visit</button>
                           </section>
                        </div>
                     </>
                  )
